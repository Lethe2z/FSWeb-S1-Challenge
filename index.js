...
function profilListesi(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    newArr[i] = arr[i].profile;
  }
  return newArr;
}
// Tum diziyi kopyaladigimizda bir performans kaybi yasiyor muyuz ? garbage collector nasil calisiyor bu noktada ?
...
function fenomenGonderimSayisi(arr, name){
  let i;
  for (let index = 0; index < arr.length; index++) {
    if(arr[index].profile === name){
      i = index;
    }
  }
  return arr[i].posts;
...
function platformaGoreCokGonderiYapanFenomen(arr, platform){
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    ...
    if (fenomen.platform === platform && fenomen.posts !== "NA" && fenomen.posts > maxPost) {
      maxPost = fenomen.posts;
      maxPoster = fenomen.profile;
    }
  }
  return maxPoster;
}
